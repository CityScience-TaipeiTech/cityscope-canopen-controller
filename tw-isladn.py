...
class CsCanOpen:
    ...
    def proximity_callback(self, msg):
        ...
        if self.send_key and (time.time() - self.start_time) > self.watting_time:
            if self.control_id != 0 and self.control_id != self.pre_control_id:
                ...
                print(self.control_id, " reset control")
                self.message_key = True
                for light_node in self.light_node_list:
        ...
        if self.map_control and (time.time() - self.start_time) > 3:
